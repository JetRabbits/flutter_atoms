...
class JsonDateTimeText {
  static DateTime? dateTimeFromString(String value, [String? pattern]) {
    ...
    if (_ == null) {
      ...
      try {
        ...
        _ = DateFormat(
                pattern ?? DateTimePatterns.SERVER_PATTERN_DATE_WITH_TIME)
            .parse(
              value,
            )
            .toLocal();
  ...
  static String toStringWithShortTime(DateTime dateTime) =>
      DateFormat(DateTimePatterns.PATTERN_DATE_WITH_SHORT_TIME)
          .format(dateTime);

  static String toStringWithTimeZone(DateTime dateTime,
      {String format = 'yyyy-MM-ddTHH:mm:ss', Duration? timeZoneOffset}) {
    Duration _timeZone;
  ...
  static String toStringWithoutTime(DateTime dateTime) =>
...
String humanReadableDatePeriod(DateTime startDate, DateTime endDate) {
  if (startDate.year == endDate.year && startDate.month == endDate.month) {
    ...
    if (startDate.day == firstDayOfMonth.day &&
        endDate.day == lastDayOfMonth.day) {
      var dateFormat = DateFormat.MMMM(Intl.getCurrentLocale());

      if (DateTime.now().year != year) dateFormat.add_y();
      var startDateString = dateFormat.format(startDate);
      return startDateString.substring(0, 1).toUpperCase() +
          startDateString.substring(1);
    }
  }

  String dateRange = JsonDateTimeText.toStringWithoutTime(startDate) +
      " ... " +
