...
class ScreenGroup {
  ...
  ScreenGroup(
      ...
      this.navBarIndex = -1,
      this.sideBarIndex = -1,
      this.navBarButtonBuilder,
      this.sideBarButtonBuilder});
  ...
  void _checkPath(NavigationScreen screen) {
    if (!screen.path.startsWith(path))
      throw "Screen path:"
          " ${screen.path} does not match screen group path "
          "$path. Please check configuration";
    if (screenMaps[screen.path] != null)
      throw "Screen ${screen.path} already added. Please check configuration";
  }
}
